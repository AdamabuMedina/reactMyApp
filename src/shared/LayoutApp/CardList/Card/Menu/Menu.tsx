...
export function Menu(props: IMenuProps) {
    const onCommentHandler = (): void => alert('comment');
    const onShareHandler = (): void => alert('share');
    const onHideHandler = (): void => alert('hide');
    const onSaveHandler = (): void => alert('save');
    const onComplainHandler = (): void => alert('complain');
    ...
    return (
        <div className={props.cssStyle}>
            <Dropdown button={
                ...
                <DropdownItems
                    title="Комментарии"
                    onItemClick={onCommentHandler}
                    additionalClass={styles.itemDropdown}
                    svgIcon={
                ...
                <DropdownItems
                    title="Скрыть"
                    onItemClick={onHideHandler}
                    additionalClass={styles.hideItemDropdown}
                    svgIcon={
                        <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
